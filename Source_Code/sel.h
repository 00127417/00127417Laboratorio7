...
Matrix createLocalK(int e,mesh &m){
    ...
    float t,k,s,y,Ae,J,D;
    
    //Componentes de K
    // [ A+K  G ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixS,matrixG,matrixD;
    ...
    //Preparando matrixS (En clase conocida simplemente como S)
    s = m.getParameter(S);
    calculateBPrima(BPrima);
    productRealMatrix((J*s)/(48*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixS);


    //Preparando matrixG (En clase conocida simplemente como G)
    y = m.getParameter(Y);
    calculateBPrima(BPrima);
    productRealMatrix((J*y)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);
    ...
    ubicarSubMatriz(K,0,5,6,8,resMatrix(matrixS,matrixG,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    productRealVector((J*w)/6,b0,b);
    b.push_back((n*J)/6); b.push_back((n*J)/6); b.push_back((n*J)/6);
